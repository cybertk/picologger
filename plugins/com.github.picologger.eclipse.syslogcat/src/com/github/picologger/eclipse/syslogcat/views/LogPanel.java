...
package com.github.picologger.eclipse.syslogcat.views;
...
import com.github.picologger.eclipse.syslogcat.ICommonAction;
import com.github.picologger.eclipse.syslogcat.views.ITableFocusListener.IFocusedTableActivator;
import com.github.picologger.eclipse.syslogcat.views.Log.LogLevel;
...
public class LogPanel extends Panel {
    ...
    /**
     * This pattern is meant to parse the first line of a log message with the
     * option
     * 'logcat -v long'. The first line represents the date, tag, severity,
     * etc.. while the
     ...
     * @param filterStorage
     *            the storage for user defined filters.
     * @param mode
     *            The filtering mode
     */
     ...
     * 
     * @param mode
     *            the column mode. Valid values are COLUMN_MOD_MANUAL and
     *            COLUMN_MODE_AUTO
     */
    ...
    /**
     * TODO:Starts a new logcat and set mCurrentLogCat as the current receiver.
     * 
     * @param device
     *            the device to connect logcat to.
     *
     */
    /*
                ...
                public void run() {
                    ...
                    try {
                        ...
                        device.executeShellCommand(
                                "logcat -v long", mCurrentLogCat, 0 /* timeout * /); //$NON-NLS-1$
                    } catch (Exception e) {
                        Log.e("Logcat", e);
                    } finally {
...
*/
    ...
    public void deleteFilter() {
        ...
        synchronized (mBuffer) {
            if (mCurrentFilter != null && mCurrentFilter != mDefaultFilter) {
                ...
                // update the content of the "other" filter to include what was
                // filtered out
     ...
     * 
     * @param filter
     *            The filter associated with the tab.
     * @param index
     *            the index of the tab. if -1, the tab will be added at the
     *            end.
     * @param fillTable
     *            If true the table is filled with the current content of
     *            the buffer.
    ...
    private TabItem createTab(LogFilter filter, int index, boolean fillTable) {
        ...
        synchronized (mBuffer) {
            ...
            // then its column
            TableColumn col = TableHelper.createTableColumn(t, "Time",
                    SWT.LEFT, "00-00 00:00:00", //$NON-NLS-1$
                    PREFS_TIME, mStore);
            if (mColumnMode == COLUMN_MODE_AUTO) {
                col.addControlListener(listener);
            }

            col = TableHelper.createTableColumn(t, "", SWT.CENTER, "D", //$NON-NLS-1$
                    PREFS_LEVEL, mStore);
            if (mColumnMode == COLUMN_MODE_AUTO) {
                col.addControlListener(listener);
            }

            col = TableHelper.createTableColumn(t, "pid", SWT.LEFT, "9999", //$NON-NLS-1$
                    PREFS_PID, mStore);
            if (mColumnMode == COLUMN_MODE_AUTO) {
                col.addControlListener(listener);
            }

            col = TableHelper.createTableColumn(t, "tag", SWT.LEFT, "abcdefgh", //$NON-NLS-1$
                    PREFS_TAG, mStore);
            if (mColumnMode == COLUMN_MODE_AUTO) {
                col.addControlListener(listener);
            }
            ...
            for (String line : lines) {
                ...
                if (line.length() > 0) {
                    ...
                    if (matcher.matches()) {
                        ...
                        mLastMessageInfo.pid = Integer
                                .valueOf(mLastMessageInfo.pidString);
                        mLastMessageInfo.logLevel = LogLevel
    ...
    private void asyncRefresh() {
        ...
        if (mFolders.isDisposed() == false) {
            synchronized (mBuffer) {
                try {
                    // the circular buffer has been updated, let have the filter
                    // flush their
