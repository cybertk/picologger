...
import com.github.picologger.eclipse.syslogcat.CommonAction;
import com.github.picologger.eclipse.syslogcat.SyslogPlugin;
import com.github.picologger.eclipse.syslogcat.views.Log.LogLevel;
import com.github.picologger.eclipse.syslogcat.views.LogPanel.ILogFilterStorageManager;

/**
 * The log cat view displays log output from the current device selection.
 *
 */
public final class LogCatView extends ViewPart {

    public static final String ID =
        "com.android.ide.eclipse.ddms.views.LogCatView"; // $NON-NLS-1$

    private static final String PREFS_COL_TIME =
        SyslogPlugin.PLUGIN_ID + ".logcat.time"; // $NON-NLS-1$
    private static final String PREFS_COL_LEVEL =
        SyslogPlugin.PLUGIN_ID + ".logcat.level"; // $NON-NLS-1$
    private static final String PREFS_COL_PID =
        SyslogPlugin.PLUGIN_ID + ".logcat.pid"; // $NON-NLS-1$
    private static final String PREFS_COL_TAG =
        SyslogPlugin.PLUGIN_ID + ".logcat.tag"; // $NON-NLS-1$
    private static final String PREFS_COL_MESSAGE =
        SyslogPlugin.PLUGIN_ID + ".logcat.message"; // $NON-NLS-1$

    private static final String PREFS_FILTERS =
        SyslogPlugin.PLUGIN_ID + ".logcat.filters"; // $NON-NLS-1$
    ...
    private final class FilterStorage implements ILogFilterStorageManager {
        ...
        public LogFilter[] getFilterFromStore() {
            String filterPrefs = SyslogPlugin.getDefault().getPreferenceStore().getString(
                    PREFS_FILTERS);
        ...
        public void saveFilters(LogFilter[] filters) {
            ...
            SyslogPlugin.getDefault().getPreferenceStore().setValue(PREFS_FILTERS, sb.toString());
        }
    ...
    public void createPartControl(Composite parent) {
        ...
        // get the font
        String fontStr = SyslogPlugin.getDefault().getPreferenceStore().getString(
    ...
    IMarker createMarkerFromSearchMatch(IFile file, SearchMatch match) {
        ...
        try {
            ...
            Status s = new Status(Status.ERROR, SyslogPlugin.PLUGIN_ID, e.getMessage(), e);
            SyslogPlugin.getDefault().getLog().log(s);
        }
        return marker;
    }
